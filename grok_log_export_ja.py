...
・チャットタイトルごとに会話ログを抽出
・月別に分割（txt / json / html）
・HTML形式でインデックス付きビューを生成
・全期間まとめファイルを出力

※ human / assistant の会話ペアのみ抽出します
※ チャット内の添付ファイルには対応していません
========================================
""")
...
title_filter = input("抽出したいチャットタイトルを入力（Enterで全件）: ").strip()
...
for idx, convo in enumerate(conversations):
    ...
    print(f"\r処理中... {percent}% ({idx+1}/{total_convos})", end="")
